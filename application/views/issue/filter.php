<style>
	@media only screen and (max-width: 600px) {
		.red-issue-filter>.red-card-content>table>tbody>tr:first-child>td {
			display: block;
		}
		.red-issue-filter>.red-card-content>table>tbody>tr:first-child>td:nth-child(-n+2){
			display: none;
		}
		.red-issue-filter>.red-card-content>table>tbody>tr>td:nth-child(1) {
			display: block;
		}
		.red-issue-filter>.red-card-content>table>tbody>tr>td:nth-child(2) {
			display: block;
		}
		.red-issue-filter>.red-card-content>table>tbody>tr>td:nth-child(2) > .j-form-field {
			padding-left: 149px;
		}

		.red-issue-filter .j-label:empty {
			display: none;
		}
	}

	.red-issue-filter .j-form-field {
		padding-right: 0;
		padding-left: 0;
	}

	.red-issue-filter>.red-card-content>table>tbody>tr>td:first-child {
		white-space: nowrap;
	}
...
<script>
	...
	function red_issue_cf_filter() {
		$.ajax({
			url: window.location.origin + ':8080/api/custom_field/list?where[main][type]=IssueCustomField&where[main][is_filter]=1&group[0]=name',
			type: 'GET',
			dataType: 'jsonp',
			success: function(data) {
				...
				$.each(param, function(i, val) {
					red_issue_add_filter_to_form(i);
				})
			},
			error: function() {},
...
<div class="red-issue-filter red-card" style="margin-top: 0px;">
	...
	<div class="red-card-content" style="overflow: auto;">
		<table class="red-layout-table" style="table-layout: auto !important;">
			<tr>
				<td>&nbsp;
				</td>
				<td>&nbsp;
			...
			<tr>
				...
				<td>
					<j-selectfield id="red-issue-status_id-value" style="display: none;" src-fn="red_status_list_value"></j-selectfield>
				</td>
			</tr>
			<!--tr>
                <td>
